"""Prepare index files for the LRS3 audio-visual dataset."""
...
def build_lrs3_index(
    ...
    *,
    ...
    for key, video_path in videos.items():
        ...
        entries.append(
            {
                "utt_id": f"lrs3-{split}-{key.replace('/', '_')}",
                "audio_ac": str(audio_path) if audio_path else None,
                "video": str(video_path),
                "text": transcript,
                "spk_id": speaker,
                "meta": {"dataset": "LRS3", "split": split},
            }
        )
    return entries
...
def main() -> None:  # pragma: no cover - CLI wrapper
    parser = argparse.ArgumentParser(description="Prepare LRS3 dataset index")
    parser.add_argument("root", type=Path, help="LRS3 dataset root")
    parser.add_argument("output", type=Path, help="Output JSONL index path")
    parser.add_argument("--split", default="trainval", help="Dataset split (trainval|pretrain|test)")
    parser.add_argument("--subset-root", type=Path, default=None, help="Optional explicit split directory")
    args = parser.parse_args()

    entries = build_lrs3_index(args.root, split=args.split, subset_root=args.subset_root)
    write_index(entries, args.output)
    print(f"Wrote {len(entries)} LRS3 entries to {args.output}")
